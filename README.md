...
**Por qué 10 y no 15:** el LP ganado/perdido por partida no viene directo de
...
10 minutos en vez de cada 15 reduce las chances de que eso pase, sobre todo
con jugadores que encadenan partidas rápido, sin subir tanto la carga sobre
la cuota de la API de Riot como un intervalo más corto (5 min). Cada corrida
ya espacia sus llamadas (`RIOT_API_REQUEST_DELAY_MS`, 100ms por defecto) y
reintenta sola ante un 429 (`riotFetch`, hasta `RIOT_FETCH_MAX_ATTEMPTS`
veces — ver `/api/update-rankings/route.ts`). Si empezás a ver muchos
`riot_api_error_429` en los logs de Vercel, o notás que a alguien no se le
actualizan los castigos/Aegis por varias corridas seguidas (señal de que el
cursor de esa persona quedó trabado en una partida que no se pudo bajar),
subí `RIOT_API_REQUEST_DELAY_MS` o volvé a un intervalo más largo.
